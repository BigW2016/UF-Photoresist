...
{
#if defined(LCD_4BIT_HIGHT) || defined(LCD_4BIT_LOW)
	//4 bit part
	_delay_ms(15);
	//zero to data pins
...
/*
	...
	while(ch<64)
	{
		...
		*/
	//мой код!!! Иначе глюки при резете МК		
		#define LCD_EntryModeSet	  0x06
		LCDsendCommand(LCD_EntryModeSet);

}	
void LCDsendChar(uint8_t ch)		//Sends Char to LCD
{

#ifdef LCD_4BIT_HIGHT
	//4 bit part
	LDP=((ch & 0b11110000));
	LCP|=1<<LCD_RS;
	LCP|=1<<LCD_E;		
